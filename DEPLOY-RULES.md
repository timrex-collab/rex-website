...
> damit das Protokoll unverändert bleibt.)*
> Reine Doku-/Tooling-PRs (#14–#17, #22–#24, #45, #46, #48, #50, #51, #53, #54, #57, #59,
> #60, #62, #64, #66, #78) sind bewusst nicht als Deploy-Zeilen geführt — sie berühren den
> Build (`dist/public`) nicht und sind nicht gate-relevant.
>
> **Stand 18.09.2026 (nach PR #78):** Letzter funktionaler Merge ist **PR #77 (VELUX D6)
> am 18.09.2026 15:24:07 MESZ**, Netlify `published_at` 15:24:49 MESZ. Die reine
> Dokumentations-PR #78 wurde am selben Tag gemergt und ging um **15:44:20 MESZ** live
> (Netlify `6aad4014…`, `commit_ref be34af8`, `state ready`, `plugin_state success`,
> Secret-Scan 717/0, Prerender-Function vorhanden, 32 Redirects, 1 Edge-Function; Netlify
> meldet „All files already uploaded by a previous deploy" — der Build ist byte-identisch
> zu #77, wie bei einer Doku-PR zu erwarten). Checks-Lauf #31 und IndexNow-Lauf #49 grün;
> der Prerender-Check läuft bei Doku-PRs pfadbedingt nicht.
> **Nächstes 48-h-Fenster:** ab **20.09.2026 15:44 MESZ**, gemessen am jüngsten
> Produktionsdeploy (#78). Gemessen am letzten funktionalen Deploy (#77) wäre es 15:24 MESZ
> — die spätere der beiden Zeiten gilt.
> **Nächster Deploy in Vorbereitung: VELUX D7** (PDF-Export an die abgesendete Anfrage
> koppeln, `VELUX-EXPORT-GATE.md`); Freigabe und Merge durch Tim.
>
> **Stand 20.09.2026 (nach PR #79, VELUX D7):** D7 ist live — Netlify `6ab03136…`,
> `commit_ref 682a888`, `published_at 21:17:45 MESZ`, `state ready`. Das Gate wurde mit
> **53 h 33 min** eingehalten; das Fenster stand seit 15:44 MESZ offen, gemergt wurde um
> 21:17 MESZ. Alle drei Nachlaufprüfungen grün (Checks #33, IndexNow #50,
> Prerender-Check #12).
> **Nächstes 48-h-Fenster:** ab **22.09.2026 21:17 MESZ**.
> **Als Nächstes vorgesehen:** F09 aus dem Förderaudit vom 17.09.2026 — der
> `ExpertenBlock` zeigt derzeit das **Builddatum** als fachliches Prüfdatum („Geprüft:
> {Monat Jahr}", `vite.config.ts:8` → `ExpertenBlock.tsx:13`) auf 13 Seiten. Jeder Deploy
> schreibt dieses Datum fort, ohne dass eine fachliche Prüfung stattfand — auch dieser hier.
> Stufe A, zwei Codedateien; der einzusetzende Prüfmonat ist von Tim zu bestimmen. Danach
> Release A des Audit-Umsetzungsplans (P1-Befunde F01–F04) nach redaktioneller und
> steuerlicher Abnahme. D8 (WebMCP-Aktivierung) bleibt davon unberührt und ist weiterhin
> nicht freigegeben.
>
...
| **VELUX D2** | Zweites von sechs Paketen der VELUX-Überarbeitung. Innenrollos (DKL/DSL) samt Montage zählen nicht mehr zur Förderbasis; förderrelevant sind nur Positionen mit Uw ≤ 1,0. BEG-Staffel mit iSFP-Bonus erst oberhalb 30.000 €; §35c nur bei Gebäude > 10 Jahre und Selbstnutzung, 7/7/6 % mit Jahreskappung. Vier Förderfragen mit Antwort „unbekannt“; fehlende Antworten werden als Annahmen ausgewiesen. Beide Förderwege erscheinen gleichrangig und nicht kombinierbar, ohne Empfehlung. Leistungsumfang, Bruttoanzeige und Texte stimmen zwischen UI und PDF überein; PDF ohne Kontaktpflicht, Eingaben im Druckfenster als Text maskiert. Demontage enthält Entsorgung; SML-Steuerung, Gerüst, Innenausbau, Elektro und Luftdichtheit sind als Ausschlüsse benannt. Regel-Prüffrist 31.12.2026: danach keine ungeprüften Förderbeträge, die Kostenschätzung bleibt nutzbar. Neue Module `client/src/lib/velux/content.ts` und `funding.ts`; `estimate:check` deckt jetzt 9 Goldwerte und 9 Förder-Grenzfälle samt Regel-Metadaten ab. Die als `LEGACY-QUIRK` markierten Altcode-Schwächen aus D1 sind damit behoben. Release-Weg: frischer Branch von `main`, genau ein Cherry-Pick des Quellcommits `41ebd1d`, Baum identisch | ✅ live (PR #72, gemergt 09.09.2026 18:13:59 MESZ; Stufe B, 8 Dateien; Gate eingehalten: **50 h 40 min** nach dem letzten Produktionsdeploy (#71, Doku) bzw. **76 h 20 min** nach dem letzten funktionalen Merge #70; Netlify `6aa185c9…`, `commit_ref 67df98e`, `published_at 18:14:39 MESZ`, `state ready`, `plugin_state success`, Prerender-Function vorhanden, Secret-Scan 693/0; IndexNow-Lauf #43 grün; Checks-Lauf #18 grün; Prerender-Check-Lauf #7 grün) — **Merge durch Tim**; WebMCP nicht aktiviert |
| **VELUX D3** | Drittes von sechs Paketen der VELUX-Überarbeitung. Der Typenschild-Resolver arbeitet gegen den Herstellerkatalog statt gegen den früheren Größen-Seed. `047` und `Y47` gehen ins manuelle Austauschprogramm und werden **nie** auf MK08 gemappt; unbelegte Zahlencodes wie `106` werden nicht erzeugt. Alt-Codes (`M08` 2000–2013, `308` 1991–2000), Bildquellen, OCR-Varianten und alternative Lesarten führen zu einer Rückfrage statt zu einem Ergebnis. Rollläden, Eindeckrahmen und Fremdhersteller → `unsupported_product`; Sondergrößen und nicht kalkulierbare Kombinationen → `manual_review`. Produktionscodes werden nie zurückgegeben. Neu: `client/src/lib/velux/resolve.ts`, `legacyCatalog.ts` und `VELUX-TYPENSCHILD.md`; `estimate:check` deckt zusätzlich 54 Größencodes / 39 Typen sowie 24 Resolverfälle mit 578 Fuzz-Kombinationen ab. Keine Foto-Uploadfunktion, keine Datenübertragung, keine sichtbare Preisänderung. Release-Weg: frischer Branch von `main`, genau ein Cherry-Pick des Quellcommits `34cc0e2`, Baum identisch | ✅ live (PR #73, gemergt 11.09.2026 21:49:03 MESZ; Stufe B, 5 Dateien; Gate eingehalten: **51 h 34 min** nach dem letzten Produktionsdeploy (#72, D2); Netlify `6aa45b30…`, `commit_ref a9db577`, `published_at 21:49:41 MESZ`, `state ready`, `plugin_state success`, Prerender-Function vorhanden, Secret-Scan 698/0; IndexNow-Lauf #44 grün; Checks-Lauf #20 grün; Prerender-Check-Lauf #8 grün) — **Merge durch Tim**; WebMCP nicht aktiviert; **nächste Freigabe (D4) frühestens 48 h nach dem Produktionsdeploy dieser Nachdokumentation** |
| **VELUX D4** | Viertes von sechs Paketen der VELUX-Überarbeitung. Der WebMCP-Adapter `client/src/lib/webmcp.ts` kommt ins Repo, **ohne aktiviert zu werden**: er prüft Flag und Origin, registriert asynchron und bindet die Abmeldung an ein Abort-Signal; der alte Navigator-Fallback entfällt, Fehler und Events enthalten keine Nutzdaten. `client/.env.example` dokumentiert die drei Build-Zeit-Variablen `VITE_WEBMCP_ENABLED` (nur exakt `true` aktiviert), `VITE_WEBMCP_ORIGINS` (Allowlist, leer = aus) und `VITE_WEBMCP_OT_TOKEN` (öffentliches Origin-Trial-Token, kein Secret) — **keine davon wurde gesetzt**. Neu als devDependencies exakt gepinnt: `playwright 1.62.1`, `webmcp-types 0.1.5`; `bufferutil` bleibt optionalDependency, das Lockfile enthält die im Hand-off §4 beschriebene Ergänzung, an der der erste Linux-Lauf scheiterte. Noch **kein Tool registriert** — das macht D5. Nachgemessen vor dem Merge: die Zeichenkette `webmcp` kommt in keinem der 65 gebauten JS-Assets vor (Gegenprobe `velux`: 14 Treffer), der Adapter wird also von nichts importiert. Release-Weg: frischer Branch von `main`, genau ein Cherry-Pick des Quellcommits `6b43ee6`, Baum identisch | ✅ live (PR #75, gemergt 14.09.2026 07:24:07 MESZ; Stufe B, 7 Dateien; Gate eingehalten: **57 h 24 min** nach dem letzten Produktionsdeploy (#74, Doku) bzw. **57 h 34 min** nach dem letzten funktionalen Deploy (#73, D3); Netlify `6aa784f9…`, `commit_ref e426f12`, `published_at 07:24:53 MESZ`, `state ready`, `plugin_state success`, Prerender-Function vorhanden, Secret-Scan 704/0; IndexNow-Lauf #46 grün; Checks-Lauf #24 grün; Prerender-Check-Lauf #9 grün) — **Merge durch Tim**; WebMCP nicht aktiviert |
| **VELUX D5** | Fünftes von sechs Paketen der VELUX-Überarbeitung. Vier Tools werden verdrahtet — `rex_velux_get_options_v1`, `rex_velux_resolve_existing_window_v1`, `rex_velux_calculate_estimate_v1`, `rex_velux_apply_configuration_v1` — alle gegen dieselbe Fachlogik wie der manuelle Rechner (`estimate.ts`, `funding.ts`, `resolve.ts`); **kein zweiter Rechenweg**. `validate.ts` prüft jede Eingabe strikt, `bridge.ts` hält die Verbindung zur UI, `apply` übernimmt erst nach bestätigtem UI-Commit und liefert bei einem zweiten Aufruf ohne `replaceExisting` `DRAFT_EXISTS` statt stillschweigend zu überschreiben. Der Eingriff in `VeluxPreisrechner.tsx` bleibt klein (+20/−2): vier `useWebMCPTool`-Aufrufe, die Bridge, ein `aria-live`-Bereich und ein Anker auf dem Ergebnisblock. **Kein Tool versendet eine Anfrage.** `estimate:check` deckt zusätzlich Validator-Fälle mit 720 Fuzz-Eingaben sowie WebMCP-Presenter, Fingerprint, Antwortgrößen und Tool-Metadaten ab. Vor dem Merge zusätzlich gefahren: `webmcp-smoke.mjs` im Shim gegen einen lokalen Preview-Build — 16 Prüfungen bestanden, darunter Goldwert 2.104 € brutto, `DRAFT_EXISTS` und **0 Netzwerk-Requests** durch Tools. Gegenprobe im Standardbuild: drei Sperren in Reihe (Flag, Origin-Allowlist, lokaler Ausschalter), ohne gesetzte Variable greift die erste mit `reason:"flag"`. Release-Weg: frischer Branch von `main`, genau ein Cherry-Pick des Quellcommits `2148fef`, Baum identisch | ✅ live (PR #76, gemergt 16.09.2026 09:22:45 MESZ; Stufe B, 8 Dateien; Gate eingehalten: **49 h 57 min** nach dem letzten Produktionsdeploy (#75, D4); Netlify `6aaa43c6…`, `commit_ref b31b092`, `published_at 09:23:20 MESZ`, `state ready`, `plugin_state success`, Prerender-Function vorhanden, Secret-Scan 711/0; IndexNow-Lauf #47 grün; Checks-Lauf #26 grün; Prerender-Check-Lauf #10 grün) — **Merge durch Tim**; WebMCP nicht aktiviert |
| **VELUX D6** | Letztes der sechs Codex-Pakete. `checks.yml` bekommt eine Browserstrecke (+47 Zeilen): Testbrowser installieren, `vite preview` starten, dann fünf Prüfungen — UI/mobile/Druck ohne Versand, WebMCP im Aus-Zustand samt Token-Gegenprobe, ein Testbuild mit aktivem Flag, der Tool-Vertrag im Shim und die Gegenprobe, dass eine **fremde Origin trotz aktivem Flag nicht registriert**. Screenshots und Druck-PDF werden als Artefakt hochgeladen (14 Tage). Ein `schedule`-Trigger (montags 07:15 UTC) lässt die Checks auch ohne Codeänderung laufen, damit eine abgelaufene Förderprüfung auffällt — Prüffrist 31.12.2026. Einzige inhaltliche Codeänderung: eine Zeile in `buildPdfHtml`, der Förderabschnitt beginnt im Druck auf eigener Seite (`break-before:page`). Dazu kommen `VELUX-HANDOFF.md`, `VELUX-HANDOFF-MANIFEST.json`, `VELUX-REVIEW-2026-09-06.md` und `CLAUDE.md` ins Repo; das Verfahren ist damit nicht mehr nur auf dem Quellbranch nachlesbar. Der Head wurde wie vom Hand-off verlangt **frisch über den Remote-Ref aufgelöst** (`dcfff57`) und gegen Elterncommit (`2148fef` = D5-Quelle), Commitzahl (1) und Dateiliste (8) geprüft. Die neue Strecke lief vor dem Merge lokal vollständig durch und anschließend auf GitHub: alle acht neuen Schritte grün, Laufzeit 90 s statt 35 s. Release-Weg: frischer Branch von `main`, genau ein Cherry-Pick, Baum identisch | ✅ live (PR #77, gemergt 18.09.2026 15:24:07 MESZ; Stufe B, 8 Dateien; Gate eingehalten: **54 h 0 min** nach dem letzten Produktionsdeploy (#76, D5); Netlify `6aad3b79…`, `commit_ref 1188307`, `published_at 15:24:49 MESZ`, `state ready`, `plugin_state success`, Prerender-Function vorhanden, Secret-Scan 717/0; IndexNow-Lauf #48 grün; Checks-Lauf #28 grün; Prerender-Check-Lauf #11 grün) — **Merge durch Tim**; WebMCP nicht aktiviert. **Damit ist die Codex-Reihe D1–D6 vollständig übernommen.** Als nächstes funktionales Paket folgt D7 (Export-Gate, `VELUX-EXPORT-GATE.md`), danach D8 (WebMCP-Aktivierung) |
| **VELUX D7** | Erstes Paket nach der Codex-Reihe, Zuschnitt aus `VELUX-EXPORT-GATE.md` (Entscheidung Tim, 09.09.2026). Die PDF-Zusammenfassung ist an eine **bestätigt versendete Anfrage** gekoppelt: freigeschaltet wird gegen `res.ok` des Netlify-Forms-POST, nicht gegen ausgefüllte Felder. Der Button „Nur PDF erstellen" entfällt; an seiner Stelle steht der gesperrte Zustand mit Begründung, nach Freischaltung „PDF erneut öffnen" (beliebig oft). Das Druckfenster wird weiterhin **im Nutzerklick** geöffnet — zunächst mit neutralem Wartescreen, damit der Popup-Blocker nicht greift — und erst nach bestätigtem Versand durch das PDF ersetzt; scheitert der Versand, schließt sich das Fenster wieder und es gibt **kein** PDF. Die Freischaltung gilt nur für die abgesendete Konfiguration (Signatur über Positionen und Förderangaben); wird danach etwas geändert, ist das Gate wieder zu. Strg+P auf der Ergebnisseite zeigt keine Zahlen mehr, sondern einen Hinweis auf den regulären Weg. Unter den CTAs steht ein Datenschutzhinweis mit Link auf `/datenschutz`. Ersatzweg bei Ausfall: der mailto-Fallback wird nicht nur angesprungen, sondern steht zusätzlich sichtbar in der Fehlermeldung — der bisher tote `buildMailto()` ersetzt dabei den zweiten, parallel gepflegten Mailtext im `catch`-Zweig. Neues Modul `client/src/lib/velux/export-gate.ts` (Gate-Zustand, Wartescreen, Texte; **keine Preis- oder Förderlogik**). Die Signatur ist normalisiert: nur preisbestimmende Felder und nach Schlüssel sortierte Förderantworten, keine internen Laufnummern — `estimate:check` prüft das direkt. Der D6-Browsertest wurde nicht umbenannt, sondern strukturell umgebaut: Der Route-Handler beantwortet den Versand lokal (200/500 je Fall) statt jeden Nicht-GET abzubrechen, die Schlussprüfung `posts === 0` weicht `posts === 2`, und sieben Fälle laufen getrennt — kein Export vor Versand, entwerteter Direktdruck, Fehlversand ohne PDF mit erreichbarem mailto, bestätigter Versand mit vollständigem PDF, erneutes Öffnen ohne weiteren POST, geänderte Konfiguration schließt das Gate, blockiertes Popup fällt auf „PDF erneut öffnen" zurück. Die alten Prüfungen (HTML-Escaping der Kundeneingabe, UI/PDF-Parität, A4-Rendering) bleiben erhalten, nur hinter dem bestätigten Versand. Bewusstes Verhalten, dokumentiert: Die Freischaltung lebt im Zustand von `Step3` und endet beim Rücksprung in den Wizard — „beliebig oft öffnen" gilt innerhalb der Ergebnisansicht. **Grenze, bewusst so dokumentiert:** Das PDF entsteht weiter im Browser; wer die Entwicklerwerkzeuge bedient, umgeht das Gate. Ziel ist nicht Geheimhaltung, sondern dass der bequeme Ein-Klick-Export an einen Lead gebunden ist. Serverseitige PDF-Erzeugung bleibt eigenes, größeres Paket | ✅ live (PR #79, gemergt 20.09.2026 21:17:08 MESZ; Stufe B, 8 Dateien; Gate eingehalten: **53 h 33 min** nach dem letzten Produktionsdeploy (#78, Doku); Netlify `6ab03136…`, `commit_ref 682a888`, `published_at 21:17:45 MESZ`, `state ready`, `plugin_state success`, Buildzeit 33 s, Prerender-Function vorhanden, Secret-Scan 720/0; IndexNow-Lauf #50 grün; Checks-Lauf #33 grün; Prerender-Check-Lauf #12 grün) — **Merge durch Claude, von Tim ausdrücklich beauftragt** (siehe §5.5-Vermerk unten); WebMCP nicht aktiviert. Vor dem Merge lokal vollständig gefahren: `npm run check`, `npm run build`, `faq:check` (23 Seiten, 0 beanstandet) samt Selbsttest, `estimate:check` (Goldwert 2.104 € brutto unverändert, Signaturregel grün), die neue UI-Strecke (7 Fälle grün), WebMCP im Aus-Zustand, WebMCP-Vertrag im Shim-Testbuild und die Gegenprobe fremde Origin; auf GitHub liefen dieselben 17 Schritte grün. **Weiterhin offen, nicht durch diesen Deploy erledigt:** die rechtliche Einordnung aus `VELUX-EXPORT-GATE.md` §3.5 („keine Vorab-Checkbox nötig") ist eine Bewertung, keine technische Feststellung, und gehört bestätigt |
...
> **Wiederholung am 01.09.2026 für PR #63** (Folgepaket §8): Tim hat die Aussetzung am
> Vortag ausdrücklich erteilt, zusammen mit dem Auftrag, das Fenster selbst abzuwarten.
> Umgesetzt als terminierte Selbstweckung um 15:00 UTC — **kein** GitHub-Auto-Merge, der
> hätte schon am Vorabend gemergt und das Gate um rund 20 Stunden unterschritten.
> Vorprüfung vor dem Merge (protokolliert): Gate nachgerechnet und die drei Merges seit #61
> (#62, #64, #66) einzeln daraufhin geprüft, dass sie `client/` nicht anfassen und den Anker
> nicht verschieben; Branch auf `main`-Spitze, Netlify-Preview grün, `npm run check`,
...
> **ebenso die gesamte VELUX-Reihe: #70 (D1), #72 (D2), #73 (D3), #75 (D4), #76 (D5)**
> **und #77 (D6, 18.09.2026 15:24:07 MESZ) — dazu die Doku-PRs #71 und #74.**
> Für #72 und #73 wurde keine Aussetzung erteilt und auch keine gebraucht: Claude hat den
> PR vorbereitet, das Fenster belegt und den Draft-Status aufgehoben, gemergt hat Tim.
...
> **Wiederholung am 20.09.2026 für PR #79 (VELUX D7):** Tim hat am 18.09.2026 ausdrücklich
> beauftragt, den Merge selbst zu übernehmen. Auch hier **kein** GitHub-Auto-Merge — die
> Checks waren seit dem 18.09. grün, ein Auto-Merge hätte sofort gemergt und das Gate um rund
> 49 Stunden unterschritten. Stattdessen zwei terminierte Selbstweckungen: 13:20 UTC zur
> Vorprüfung, 13:50 UTC zur Fensteröffnung. Protokollierte Vorprüfung 24 min vor dem Fenster:
> Gate über die Netlify-API neu berechnet statt aus der Dokumentation übernommen (jüngster
> Produktionsdeploy weiterhin #78, `published_at 18.09. 13:44:20 UTC`), `main` unverändert
> auf `be34af8`, PR `mergeable_state: clean`, Head `59f0d7b` unverändert, alle vier
> Check-Runs grün, Preview-Deploy `ready`. Eine Besonderheit gegenüber #63: Der geplante
> Merge-Auftrag wurde vom Auto-Mode-Klassifikator der Session abgelehnt („Merge Without
> Review"), unabhängig von Tims Aussetzung. Deshalb wurde die Freigabe zur Fensteröffnung
> erneut eingeholt und erst danach gemergt — um 21:17:08 MESZ, **53 h 33 min** nach #78, im
> regulären Fenster und ohne Gate-Ausnahme nach Abschnitt 4. Zwischen Freigabefrage und
> Antwort lagen rund fünfeinhalb Stunden; der Abstand fiel dadurch größer aus als die bei
> der Anfrage genannten 48 h 7 min.
> **Reichweite:** Die Aussetzung gilt jeweils **pro PR** und wurde bisher für #56, #61, #63,
> #67, #68, #69 und #79 erteilt — **fünf davon Doku- oder Tooling-PRs, zwei echte Deploys**
> (#63 und #79). Sie ist keine Dauerregel; für jeden weiteren Deploy gilt §5.5 unverändert,
