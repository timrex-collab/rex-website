...
export function presentEstimate(input: EstimateInput, estimate: CanonicalEstimate, calculatedAt: string = new Date().toISOString()) {
  ...
  return {
    ...
    funding: {
      ...
      notes: ([funding.ineligibleThermoPositions > 0 ? FUNDING_NOTES.thermo : null, FUNDING_NOTES.notCombinable, FUNDING_NOTES.rounding] as Array<string | null>).filter((n): n is string => n !== null),
    },
...
export function buildResolveTool(): WebMCPTool {
  return {
    ...
    // Lesarten und Unsicherheitsgründe stammen aus Nutzereingaben.
