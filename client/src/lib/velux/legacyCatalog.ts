 ...
 * gegen die VELUX-Größentabelle geprüfte Einträge tragen
 * `verification: "verified"`. Beides ändert nichts daran, dass Alt-Codes immer
 * bestätigt werden müssen.
 *
 * Quellen: VELUX kat 5 (03.09.2026), S. 119–123; Dokument 7061, S. 2/6/10/13.
 * Der frühere Händler-Seed ist ersetzt. Systematik:
 *   ab 2013      zwei Buchstaben + zwei Ziffern  (MK08)   – „K"-Generation
 *   2000–2013    ein Buchstabe + zwei Ziffern    (M08)
 *   1991–2000    explizit belegte dreistellige Codes (z. B. 308)
 *   VU/VKU-Y     eigenes Austauschprogramm, immer manuelle Prüfung
...
export const LEGACY_CATALOG_VERSION = "2026-09-06.1";
...
// Explizit im Herstellerkatalog kat 5, gedruckte S. 123, ausgewiesen.
// 106, 208 und 804 dürfen NICHT aus einer Zahlenregel erfunden werden.
const VERIFIED_NUMERIC = new Set(["102", "104", "204", "206", "304", "306", "308", "310", "406", "408", "410", "606", "608", "610", "808", "810"]);
export const LEGACY_EXCHANGE_CODES = new Set(["021", "023", "033", "035", "043", "045", "047", "065", "067", "085", "087", "089", "107", "109", "Y21", "Y23", "Y33", "Y35", "Y43", "Y45", "Y47", "Y65", "Y67", "Y85", "Y87", "Y89", "Y97", "Y99"]);
...
function buildSizeEntries(): LegacySizeEntry[] {
  ...
  for (const current of SIZE_CODES) {
    ...
    entries.push({ observedCode: `${letter}${height}`, generation: "2001_2013", legacySize: `${letter}${height}`, currentEquivalent: current, mapping: "legacy_equivalent", verification: "verified", sourceRef: "VELUX kat 5, 03.09.2026, S. 123: Größenraster 2000–2013 (Generationsschlüssel historisch)" });
    const digit = Object.entries(WIDTH_LETTER).find(([, l]) => l === letter)?.[0];
    if (digit && VERIFIED_NUMERIC.has(`${digit}${height}`)) {
      entries.push({ observedCode: `${digit}${height}`, generation: "1991_2001", legacySize: `${digit}${height}`, currentEquivalent: current, mapping: "legacy_equivalent", verification: "verified", sourceRef: "VELUX kat 5, 03.09.2026, S. 123: Größenraster 1991–2000; Innenfutter und Einbautiefe separat prüfen" });
    }
  }
  // 047 = 74×143 cm, Y47 = ca. 75×144 cm; MK08 = 78×140 cm.
  // Das VU/VKU-Y-Programm ist keine Standardgröße (Katalog S. 121).
  return entries;
}
...
export const WINDOW_TYPES: readonly WindowTypeEntry[] = [
  ...
  { code: "VL", category: "roof_window", label: "Schwingfenster (Altprogramm bis 2001)", supported: false, note: "VU/VKU-Austauschprogramm oder Sondermaß; Typ, Innenfutter und Einbautiefe manuell prüfen." },
  ...["VG", "VX", "VK", "VKU", "VLPU", "VS", "VT", "VH"].map((code): WindowTypeEntry => ({ code, category: "roof_window", label: "VELUX Alt-/Sonderprogramm", supported: false, note: "Austauschprogramm manuell prüfen; Ausstiegs-, Klapp- und Schallschutzfunktionen nicht automatisch ersetzen (Katalog S. 119–121)." })),
