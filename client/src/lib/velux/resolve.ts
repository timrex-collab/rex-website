...
import {
  LEGACY_CATALOG_VERSION, LEGACY_EXCHANGE_CODES, OTHER_BRANDS, findLegacySize, findWindowType, isSupportedModel, looksLikeCurrentSizeCode,
...
function lookupSize(code: string): SizeLookup {
  ...
  if (LEGACY_EXCHANGE_CODES.has(code)) return { current: null, catalogStatus: "unsupported", legacySize: code, generation: "unknown", mapping: "unsupported", reason: "VU/VKU-Austauschprogramm mit abweichenden Maßen. Kein MK-/K-Äquivalent; Innenfutter, Einbautiefe und Sonderfunktion manuell prüfen." };
  ...
  if (looksLikeCurrentSizeCode(code)) {
    return { current: null, catalogStatus: "unsupported", legacySize: null, generation: "2013_plus", mapping: "unsupported", reason: `Größe ${code} ähnelt dem aktuellen Codeschema, ist aber nicht im geprüften Rechnerkatalog enthalten.` };
  }
...
export function describeGeneration(g: Generation): string {
  return { pre_1991: "Altprogramm", "1991_2001": "1991–2000", "2001_2013": "2000–2013", "2013_plus": "ab 2013", unknown: "Generation unbekannt" }[g];
}
...
export function resolveExistingWindow(input: WindowObservation): ResolveResult {
  ...
  if (brand === "VELUX") brandField.catalogStatus = "exact";
  else if (!brand) { brandField.catalogStatus = "missing"; warnings.push("Hersteller nicht angegeben – VELUX angenommen."); }
  else { brandField.needsUserConfirmation = true; return finish("manual_review", null, "Hersteller nicht eindeutig als VELUX erkannt. Bitte Typenschild prüfen; keine automatische Herstellerannahme."); }
  ...
  const hasAlternatives = typeAlts.length > 0 || sizeAlts.length > 0 || brandField.alternatives.length > 0 || Boolean(typeObs.reason || sizeObs.reason || brandObs.reason);
  const isLegacy = sizeLookup.catalogStatus === "legacy_equivalent";
  if (isLegacy) {
    sizeField.needsUserConfirmation = true;
    warnings.push(sizeLookup.reason as string);
    warnings.push("Größenraster bestätigt keinen 1:1-Austausch: Einbautiefe, Eindeckrahmen und Innenfutter prüfen. Neue SSL/SML-K-Rollläden passen nicht zur Nachrüstung auf Fenster der Generation 102–810; der Rechner kalkuliert Zubehör für das neue Fenster.");
  }
  if (fromImage) { typeField.needsUserConfirmation = sizeField.needsUserConfirmation = true; }
  if (hasAlternatives) {
    typeField.needsUserConfirmation ||= typeAlts.length > 0 || Boolean(typeObs.reason);
    sizeField.needsUserConfirmation ||= sizeAlts.length > 0 || Boolean(sizeObs.reason);
    brandField.needsUserConfirmation ||= brandField.alternatives.length > 0 || Boolean(brandObs.reason);
  }
