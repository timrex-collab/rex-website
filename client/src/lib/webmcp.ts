...
export function getModelContext(): WebMCP.ModelContext | null {
  ...
  // Kein Navigator-Fallback auf eine ältere API mit anderem Lebenszyklus.
  const mc = document.modelContext;
  return mc && typeof mc.registerTool === "function" && typeof mc.getTools === "function" && "executeTool" in mc && typeof mc.executeTool === "function" ? mc : null;
}
...
export function combineSignals(a: AbortSignal, b: AbortSignal | undefined): { signal: AbortSignal; dispose: () => void } {
  const noop = () => {};
  if (!b) return { signal: a, dispose: noop };
  const anyFn = (AbortSignal as unknown as { any?: (signals: AbortSignal[]) => AbortSignal }).any;
  if (typeof anyFn === "function") return { signal: anyFn([a, b]), dispose: noop };
  const c = new AbortController();
  const onAbort = () => c.abort();
  if (a.aborted || b.aborted) c.abort();
  a.addEventListener("abort", onAbort, { once: true });
  b.addEventListener("abort", onAbort, { once: true });
  return { signal: c.signal, dispose: () => { a.removeEventListener("abort", onAbort); b.removeEventListener("abort", onAbort); } };
}
...
export async function registerWebMCPTool(tool: WebMCPTool, lifecycle: AbortSignal): Promise<RegistrationResult> {
  ...
  const wrapped: WebMCPTool = {
    ...
    execute: async (input, options) => {
      ...
      const { signal, dispose } = combineSignals(lifecycle, options?.signal);
      ...
      try {
        ...
        // Exceptions können Eingaben enthalten: nie das Fehlerobjekt loggen.
        console.error(`[webmcp] ${tool.name}: INTERNAL_ERROR`);
  ...
  try {
    ...
    if (lifecycle.aborted) return report({ status: "registration_aborted" });
    return report({ status: "registration_error", error: "REGISTRATION_FAILED" });
  }
  if (lifecycle.aborted) return report({ status: "registration_aborted" });
