...
function buildPdfHtml(details, totals, funding, kunde) {
  // Die Druckansicht enthält Kundeneingaben als Text, niemals als HTML.
  const escapeHtml=(value)=>String(value??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
  kunde=Object.fromEntries(Object.entries(kunde).map(([key,value])=>[key,escapeHtml(value)]));
  ...
  const posRows = details.map((d, i) => {
    ...
    const labLines = [
      `${d.qty}× Demontage Altfenster inkl. Entsorgung|ab ${fmt(LABOR.demontage)} €`,
...
<style>
  ...
  .foerder-box,.total-section,.kunde-box,.disclaimer{break-inside:avoid;page-break-inside:avoid}
...
<div class="page">
  ...
  <div class="disclaimer">
    ...
    ${DISCLAIMER}<br>${FUNDING_NOTES.rounding}<br>
...
function Step1({positions,setPositions}){
  ...
  const add=()=>setPositions(ps=>ps.length<10?[...ps,emptyPos()]:ps);
  return(<div className="space-y-4">
    ...
    <button onClick={add} disabled={positions.length>=10} className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 hover:border-slate-400 hover:text-slate-700 transition-all text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"><Plus className="w-4 h-4"/> {positions.length>=10?"Maximal 10 Positionen – für größere Vorhaben bitte anfragen":"Weitere Fenstergröße hinzufügen"}</button>
  </div>);
}
...
function Step2({foerderung,setFoerderung}){
  ...
  return(<div className="space-y-5">
    ...
    {qs.map((q,i)=>(<div key={q.key} className="bg-white border border-slate-200 rounded-xl p-5">
      ...
      <div className="flex flex-wrap gap-3" role="group" aria-label={q.label}>{q.options.map(([v,l])=>(<button key={v} aria-pressed={foerderung[q.key]===v} onClick={()=>setFoerderung(f=>({...f,[q.key]:v}))}
...
function Step3({positions,foerderung}){
  ...
  return(
    <div className="space-y-5">
      {/* Config Summary */}
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-5 text-white outline-none focus-visible:ring-2 focus-visible:ring-white/60">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        {details.map((d,i)=>(
          <div key={i} className={`p-4 ${i>0?"border-t border-slate-100":""}`}>
            ...
            <div className="space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-slate-500">{d.qty}× Demontage Altfenster inkl. Entsorgung</span><span className="text-slate-500">ab {fmt(LABOR.demontage)} €</span></div>
  ...
  return(
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <div className="bg-slate-900 text-white"><div className="max-w-3xl mx-auto px-4 py-8 sm:py-10 text-center">
        ...
        <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">VELUX Dachfenster Preisrechner</h2>
