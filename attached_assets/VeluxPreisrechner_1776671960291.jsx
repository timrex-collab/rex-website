...
function Step3({positions,foerderung}){
  ...
  const handlePdfAndMail=useCallback(()=>{handlePdf();setTimeout(()=>{window.location.href=buildMailto();},800);},[handlePdf,buildMailto]);
  ...
  return(
    <div className="space-y-5">
      ...
      <div className="bg-white border-2 border-slate-200 rounded-2xl p-6 space-y-4">
        <div className="text-center"><div className="text-lg font-bold text-slate-900 mb-1">Angebot anfragen</div>
          <p className="text-sm text-slate-500 mb-5">Die PDF-Kostenschätzung wird automatisch erstellt. Bitte fügen Sie sie als Anhang in der E-Mail bei.</p></div>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          {kundeValid?(<>
            <button onClick={handlePdfAndMail} className="inline-flex items-center gap-2 bg-slate-800 text-white px-7 py-3.5 rounded-xl font-semibold text-sm hover:bg-slate-700 transition-all shadow-lg hover:shadow-xl group"><Mail className="w-5 h-5"/>PDF erstellen & E-Mail senden<ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform"/></button>
            <button onClick={handlePdf} className="inline-flex items-center gap-2 bg-white text-slate-700 border-2 border-slate-300 px-6 py-3.5 rounded-xl font-semibold text-sm hover:bg-slate-50 hover:border-slate-400 transition-all"><FileDown className="w-4 h-4"/>Nur PDF erstellen</button>
          </>):(<div className="inline-flex items-center gap-2 bg-slate-200 text-slate-400 px-8 py-3.5 rounded-xl font-semibold text-sm cursor-not-allowed"><Mail className="w-5 h-5"/>Bitte zuerst Kontaktdaten ausfüllen</div>)}
        </div>
        {pdfGenerated&&(<div className="text-center animate-fadeIn"><p className="text-xs text-emerald-700 bg-emerald-50 inline-block px-4 py-2 rounded-lg"><Check className="w-3.5 h-3.5 inline mr-1"/>PDF wurde erstellt — bitte im Druckdialog als PDF speichern und der E-Mail beifügen.</p></div>)}
        <div className="text-center pt-2"><div className="flex items-center justify-center gap-2 text-sm text-slate-500"><Phone className="w-4 h-4"/><span>Oder direkt anrufen:</span><a href="tel:+49234583100" className="font-semibold text-slate-800 hover:underline">0234 / 58 31 00</a></div></div>
      </div>
    </div>);
}
